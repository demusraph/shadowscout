...
def generate_standalone_scraper(
    ...
    # 2. Serialize parameters and headers cleanly
...
class DirectScraper:
    ...
    async def fetch_page(
        ...
        """Fetches a single page with exponential retry on 429 or server errors."""
        ...
        for attempt in range(3):
            try:
                ...
                if resp.status_code == 200:
                    payload = resp.json()
                    return extract_items_from_payload(payload, ARRAY_KEY_PATH)
    ...
    async def run(
        ...
        """Runs the scraping loop and validates items against ScrapedItem model."""
        all_items: List[ScrapedItem] = []

        async with httpx.AsyncClient(follow_redirects=True) as client:
            for page in range(1, max_pages + 1):
                raw_items = await self.fetch_page(client, page_num=page, limit_size=limit_size)
                if not raw_items:
